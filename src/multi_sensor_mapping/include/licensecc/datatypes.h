...
typedef enum {
  ...
  FILE_FORMAT_NOT_RECOGNIZED =
      4, // license file has invalid format (not .ini file)
  LICENSE_MALFORMED =
      5, // some mandatory field are missing, or data can't be fully read.
  ...
  IDENTIFIERS_MISMATCH =
      9, // Calculated identifier and the one provided in license didn't match

  LICENSE_SPECIFIED = 100, // license location was specified
  LICENSE_FOUND =
      101, // License file has been found or license data has been located
  PRODUCT_FOUND =
      102, // License has been loaded and the declared product has been found
...
/**
 * This structure contains informations on the raw license data. Software
 * authors can specify the location of the license file or its full content.
 *
...
/**
 * Informations about the software requesting the license verification (eg,
 * software version, feature to verify).
 */
typedef struct {
  /**
  ...
  /**
   * Name of the feature you want to verify. If empty ('\0') the 'default'
   * feature will be verified. (every project has a default feature that is
   * equal to the project name). Every feature has a separate section in the
   * license file: <pre> [feature_xx] sig=AAAA [another_feature]
   * expiry-date=20201111
   * </pre>
   */
  char feature_name[LCC_API_FEATURE_NAME_SIZE + 1];
  /**
   * this number passed in by the application must correspond to the magic
   * number used when compiling the library. See cmake parameter
   * -DLCC_PROJECT_MAGIC_NUM and licensecc_properties.h macro VERIFY_MAGIC
   */
  unsigned int magic;
} CallerInformations;

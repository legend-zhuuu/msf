...
void PutMappingProgress(const double _progress) {
  std::cout << "Mapping Progress : " << _progress << std::endl;
}

int main(int argc, char** argv) {
  ros::init(argc, argv, "demo_lidar_imu_mapping");
  ros::NodeHandle nh;
  ...
  mapper.RegMappingProgressCallback(
      std::bind(PutMappingProgress, std::placeholders::_1));

  mapper.Start(bag_path);
  ros::spin();

  auto session = mapper.GetLidarMapSession();
  mapper.SaveMapSession();

...
bool MarginalizationInfo::Marginalize() {
  ...
  for (int i = 0; i < NUM_THREADS; i++) {
    ...
    if (ret != 0) {
      AWARN_F("[MarginalizationInfo]pthread_create error");
  ...
  Eigen::VectorXd Amm_S_inv_trunc =
      Eigen::VectorXd((Amm_S.array() > eps).select(Amm_S.array().inverse(), 0));
  Eigen::MatrixXd Amm_inv = saes.eigenvectors() * Amm_S_inv_trunc.asDiagonal() *
  ...
  // LOG(INFO) << "marginalize costs " << time[0] << "/" << time[1] - time[0]
  //           << "/" << time[2] - time[1] << "/" << time[3] - time[2] << "/"
  //           << time[4] - time[3] << " = " << time[4] << " ms.";
...
MarginalizationFactor::MarginalizationFactor(
    const MarginalizationInfo::Ptr &_marginalization_info)
...
bool MarginalizationFactor::Evaluate(double const *const *parameters,
  ...
  for (int i = 0;
    ...
    if (size != 4)
      ...
      Eigen::Quaterniond q0_inv =
          Eigen::Quaterniond(x0(3), x0(0), x0(1), x0(2)).inverse();
